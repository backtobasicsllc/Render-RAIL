...
async function runChat({ keys, system, messages }) {
  if (!keys.anthropic) throw new Error('No Anthropic API key connected — open Settings.');
  const data = await fetchJson('https://api.anthropic.com/v1/messages', {
    method: 'POST',
    headers: {
...
module.exports = { runNano, runGptImage, runVeo, runGrok, runHeygen, runHeygenAv4, runEleven, heygenCatalog, elevenCatalog, elevenVoiceSettings, runDirector, runChat };
